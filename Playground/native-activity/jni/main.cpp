...
public:
	...
	void init(int32_t width, int32_t height, function_t<string (const char*)> fileLoader)
	{
		// engine
...
/**
* This is the function to copy file from asset to the application folder
*/
string copyFileFromAsset(struct engine* engine, const char* file) {
	ANativeActivity* nativeActivity = engine->app->activity;                  
	const char* internalPath = nativeActivity->internalDataPath;
	string dataPath(internalPath);                               
	...
	if (0 == res)
	{
		// test to see if the file is already present
		res = stat(filePath.c_str(), &sb);
		...
		else
		{
			...
			if (NULL == file)
			{
				LOGW("Could not create app configuration file.\n");
			}
			else
			{
				LOGI("App config file created successfully. Writing config data ...\n");
				res = fwrite(fileData, sizeof(char), fileLen, file);
				if (fileLen != res)
				{
					LOGW("Error generating app configuration file.\n");
				}
			}
