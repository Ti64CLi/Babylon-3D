...
private dynamic _textures=new Array();
private dynamic _floats=new Array();
private dynamic _floatsArrays=new dynamic();
private dynamic _colors3=new Array();
private dynamic _colors4=new Array();
private dynamic _vectors2=new Array();
private dynamic _vectors3=new Array();
private dynamic _matrices=new Array();
private dynamic _cachedWorldViewMatrix=new BABYLON.Matrix();
public ShaderMaterial(string name, Scene scene, string shaderPath, dynamic options) {
base(name, scene);
this._shaderPath=shaderPath;
options.needAlphaBlending=options.needAlphaBlending||false;
options.needAlphaTesting=options.needAlphaTesting||false;
options.attributes=options.attributes||new Array<object>();
options.uniforms=options.uniforms||new Array<object>();
...
private virtual void _checkUniform(dynamic uniformName) {
...
public virtual ShaderMaterial setFloats(string name, float[] value) {
this._checkUniform(name);
