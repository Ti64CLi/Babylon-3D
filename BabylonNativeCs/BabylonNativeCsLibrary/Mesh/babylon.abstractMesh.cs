...
private static dynamic _BILLBOARDMODE_NONE=0;
private static dynamic _BILLBOARDMODE_X=1;
private static dynamic _BILLBOARDMODE_Y=2;
private static dynamic _BILLBOARDMODE_Z=4;
private static dynamic _BILLBOARDMODE_ALL=7;
public dynamic position=new BABYLON.Vector3(0, 0, 0);
public dynamic rotation=new BABYLON.Vector3(0, 0, 0);
public Quaternion rotationQuaternion;
public dynamic scaling=new BABYLON.Vector3(1, 1, 1);
public dynamic billboardMode=BABYLON.AbstractMesh.BILLBOARDMODE_NONE;
public dynamic visibility=1.0;
public dynamic infiniteDistance=false;
public dynamic isVisible=true;
public dynamic isPickable=true;
public dynamic showBoundingBox=false;
public dynamic showSubMeshesBoundingBox=false;
public dynamic onDispose=null;
public dynamic checkCollisions=false;
public Skeleton skeleton;
public dynamic renderingGroupId=0;
public Material material;
public dynamic receiveShadows=false;
public ActionManager actionManager;
public dynamic useOctreeForRenderingSelection=true;
public dynamic useOctreeForPicking=true;
public dynamic useOctreeForCollisions=true;
public float layerMask=0xFFFFFFFF;
public dynamic _physicImpostor=PhysicsEngine.NoImpostor;
...
public dynamic ellipsoidOffset=new BABYLON.Vector3(0, 0, 0);
private dynamic _collider=new Collider();
private dynamic _oldPositionForCollisions=new BABYLON.Vector3(0, 0, 0);
private dynamic _diffPositionForCollisions=new BABYLON.Vector3(0, 0, 0);
private dynamic _newPositionForCollisions=new BABYLON.Vector3(0, 0, 0);
private dynamic _localScaling=BABYLON.Matrix.Zero();
private dynamic _localRotation=BABYLON.Matrix.Zero();
private dynamic _localTranslation=BABYLON.Matrix.Zero();
private dynamic _localBillboard=BABYLON.Matrix.Zero();
private dynamic _localPivotScaling=BABYLON.Matrix.Zero();
private dynamic _localPivotScalingRotation=BABYLON.Matrix.Zero();
private dynamic _localWorld=BABYLON.Matrix.Zero();
private dynamic _worldMatrix=BABYLON.Matrix.Zero();
private dynamic _rotateYByPI=BABYLON.Matrix.RotationY(Math.PI);
private dynamic _absolutePosition=BABYLON.Vector3.Zero();
private dynamic _collisionsTransformMatrix=BABYLON.Matrix.Zero();
private dynamic _collisionsScalingMatrix=BABYLON.Matrix.Zero();
public Vector3[] _positions;
private dynamic _isDirty=false;
public BoundingInfo _boundingInfo;
private dynamic _pivotMatrix=BABYLON.Matrix.Identity();
public dynamic _isDisposed=false;
public dynamic _renderId=0;
public SubMesh[] subMeshes;
public Octree<SubMesh> _submeshesOctree;
public dynamic _intersectionsInProgress=new Array();
...
public virtual Matrix computeWorldMatrix(bool force) {
...
public virtual bool intersectsMesh(AbstractMesh mesh, bool precise) {
...
public virtual void setPhysicsState(object impostor, PhysicsBodyCreationOptions options) {
...
public virtual Octree<SubMesh> createOrUpdateSubmeshesOctree(dynamic maxCapacity, dynamic maxDepth) {
if (!this._submeshesOctree) 
{
this._submeshesOctree=new BABYLON.Octree(Octree.CreationFuncForSubMeshes, maxCapacity, maxDepth);
}
...
public virtual PickingInfo intersects(Ray ray, bool fastCheck) {
...
public virtual AbstractMesh clone(string name, Node newParent, bool doNotCloneChildren) {
return null;
}
