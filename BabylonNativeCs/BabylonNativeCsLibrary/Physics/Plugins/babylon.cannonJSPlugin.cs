...
var CANNON;
var window;
public class CannonJSPlugin: IPhysicsEnginePlugin {
public Func<float, float> checkWithEpsilon;
private object _world;
dynamic _registeredMeshes=new Array<object>();
dynamic _physicsMaterials=new Array<object>();
public virtual void initialize(float iterations) {
this._world=new CANNON.World();
this._world.broadphase=new CANNON.NaiveBroadphase();
this._world.solver.iterations=iterations;
}
private virtual float _checkWithEpsilon(float value) {
return (value<BABYLON.PhysicsEngine.Epsilon) ? BABYLON.PhysicsEngine.Epsilon : value;
}
public virtual void runOneStep(float delta) {
...
public virtual object registerMesh(AbstractMesh mesh, float impostor, PhysicsBodyCreationOptions options) {
...
private virtual object _createSphere(float radius, AbstractMesh mesh, PhysicsBodyCreationOptions options) {
...
private virtual object _createBox(float x, float y, float z, AbstractMesh mesh, PhysicsBodyCreationOptions options) {
...
private virtual object _createPlane(AbstractMesh mesh, PhysicsBodyCreationOptions options) {
...
private virtual object _createConvexPolyhedron(float[] rawVerts, float[] rawFaces, AbstractMesh mesh, PhysicsBodyCreationOptions options) {
var verts = new Array<object>()faces = new Array<object>();
mesh.computeWorldMatrix(true);
...
private virtual void _addMaterial(float friction, float restitution) {
var index;
var mat;
...
private virtual object _createRigidBodyFromShape(object shape, AbstractMesh mesh, float mass, float friction, float restitution) {
...
this._registeredMeshes.push(new dynamic());
return body;
}
public virtual object registerMeshesAsCompound(PhysicsCompoundBodyPart[] parts, PhysicsBodyCreationOptions options) {
...
private virtual void _unbindBody(dynamic body) {
for (var index = 0;index<this._registeredMeshes.length;index++) 
{
var registeredMesh = this._registeredMeshes[index];
