...
dynamic _vertexStrideSize=15*4;
...
public SpriteManager(string name, string imgUrl, float capacity, float cellSize, Scene scene, float epsilon) {
...
this._vertexDeclaration=new Array<object>();
...
this._effectBase=this._scene.getEngine().createEffect("sprite", new Array<object>(), new Array<object>(), new Array<object>(), "\"");
this._effectFog=this._scene.getEngine().createEffect("sprite", new Array<object>(), new Array<object>(), new Array<object>(), "#define FO");
}
private virtual void _appendSpriteVertex(float index, Sprite sprite, float offsetX, float offsetY, float rowSize) {
...
effect.setTexture("diffuseSample", this._spriteTexture);
effect.setMatrix("vie", viewMatrix);
effect.setMatrix("projectio", this._scene.getProjectionMatrix());
effect.setFloat2("textureInfo", this.cellSize/baseSize.width, this.cellSize/baseSize.height);
if (this._scene.fogMode!=BABYLON.Scene.FOGMODE_NONE) 
{
effect.setFloat4("vFogInfo", this._scene.fogMode, this._scene.fogStart, this._scene.fogEnd, this._scene.fogDensity);
effect.setColor3("vFogColo", this._scene.fogColor);
}
engine.bindBuffers(this._vertexBuffer, this._indexBuffer, this._vertexDeclaration, this._vertexStrideSize, effect);
effect.setBool("alphaTes", true);
engine.setColorWrite(false);
engine.draw(true, 0, max*6);
engine.setColorWrite(true);
effect.setBool("alphaTes", false);
